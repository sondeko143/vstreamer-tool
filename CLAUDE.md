...
# Run the GUI control panel (remote control for pipelines that are already
# running; reads its target list from the OS config dir — it never launches or
# edits a pipeline, so it takes no config-file argument)
uv run python -m gui                    # optionally: --config-dir PATH  -t THEME
...
### GUI = リモート操作パネルだけ
The `gui/` package is a remote control for pipelines that are **already running**, on this machine or another ([ADR-0060](docs/adr/0060-gui-remote-control-panel.md)). It sends exactly four control events over gRPC — ping (疎通確認) / pause / resume / reload — and holds nothing but the target list (`~/.config/vstreamer/targets.toml`: name, host, port, and the reload config path). It does **not** launch pipelines, edit config, or evaluate readiness; those layers were deleted, superseding ADR-0032/0033/0045/0046. Consequently `gui/` depends only on `EventType` and `EventAddress`, never on the `Config` schema — keep it that way, or the panel starts drifting behind config changes again. Build every Command through `EventAddress.to_pb()` rather than importing protobuf `PAUSE`/`RESUME` directly, so the `EventType` ↔ `Operation` mapping stays in one place. Two traps: `reload`'s `file_path` is opened by the **receiving** machine (never resolve or validate it locally), and every RPC needs a deadline plus an off-thread send or an unreachable host freezes the window. `vspeech/preflight.py` is unaffected — it still fails loud at startup ([ADR-0038](docs/adr/0038-worker-config-preflight-fail-loud.md)), which is now the only place config problems surface.
