...
The device layer itself is torch-free ([ADR-0078](docs/adr/0078-torch-free-device-resolution.md)): GPUs are enumerated through the CUDA driver API (`nvcuda.dll` via ctypes, in [`vspeech/lib/cuda_driver.py`](vspeech/lib/cuda_driver.py)) rather than NVML, because the ordinal it yields is handed straight to ctranslate2's `device_index` and the CUDA EP's `device_id`, which speak the CUDA ordinal space. torch enters only inside the conversion path, at the single `_torch_device` boundary in `rvc.py`. `tests/test_forbidden_imports.py` pins that the device layer imports no torch — keep it that way, or the whisper pipeline silently pays 477MB and 3.2s again. fp16 capability comes from the device's compute capability, not its product name ([ADR-0079](docs/adr/0079-fp16-by-compute-capability.md)).
