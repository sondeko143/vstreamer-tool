...
### GUI の起動前 readiness
The `gui/` control panel shows, before you press Start, whether a pipeline can actually run — but it **holds no knowledge of what each worker requires**. It calls `collect_problems` in [`vspeech/preflight.py`](vspeech/preflight.py) (the same function the startup preflight raises on) and renders the result; that module is the single authority ([ADR-0045](docs/adr/0045-gui-readiness-reuses-preflight.md)). Add a new required resource in `preflight.py` only and the GUI follows automatically — never duplicate a required-field list into `gui/`, or the panel and the real startup check drift ("green in the GUI, dies on launch"). `ConfigProblem.field` is the public contract the GUI reads to jump to the offending setting. Machine-global asset paths are edited once in `default.toml` and explicitly propagated into each pipeline's own toml, which stays independently launchable ([ADR-0046](docs/adr/0046-gui-shared-asset-paths-explicit-propagate.md), [ADR-0032](docs/adr/0032-gui-multi-pipeline-rewrite.md)).
