...
def main() -> None:
    ...
    # [Open, deferred 2026-08-12 -- outside ADR-0080's scope] This artifact carries no
    # provenance. Its streaming sibling (scripts/stream_vc_baseline.py) records the [rvc]
    # config and which package supplied each loaded CUDA library, and refuses to judge
    ...
    # Deferred because adding the record means re-capturing the golden on real hardware,
    # and this branch's acceptance criterion is bit equality against the pre-change
    # implementation -- a re-capture cannot ride along with it. Do it at the next
    # re-capture, and mirror `provenance()` / `provenance_mismatches()` from
    # scripts/stream_vc_baseline.py rather than inventing a second shape for the same job.
