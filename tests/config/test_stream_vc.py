...
def test_stream_vc_defaults():
    ...
    # Local and in-process until a role says otherwise (ADR-0055): a config that names
    # no role must still be the single-machine pipeline it was before the split.
    assert c.role is StreamVcRole.local
    assert c.transport_type == TransportType.in_process
    assert c.jitter_buffer_ms == 0.0
...
def test_producer_parses_udp_and_peer():
...
def test_consumer_parses_bind_and_jitter():
    sv = Config.model_validate(
        {
            "stream_vc": {
                "role": "consumer",
...
def test_envelope_defaults_off():
    sv = Config().stream_vc
...
def test_envelope_min_gt_max_rejected():
    import pytest
    from pydantic import ValidationError
