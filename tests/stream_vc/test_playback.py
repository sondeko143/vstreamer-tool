...
def test_a_packet_at_another_rate_is_converted_to_the_device_rate() -> None:
    ...
    sink = _sink(DEVICE_RATE)
    pcm = sine(PACKET_RATE, 8000)
    out = sink.convert(pcm, PACKET_RATE)
    assert i16(out).size == 8000 * DEVICE_RATE // PACKET_RATE
    assert abs(peak_frequency(out, DEVICE_RATE) - 440.0) < 2.0
    ...
    assert abs(peak_frequency(out, PACKET_RATE) - 440.0) > 100.0
...
def test_the_filter_state_carries_across_packet_boundaries() -> None:
    ...
    such seam exists.
    """
    whole = sine(PACKET_RATE, 16000)
    packets = [i16(whole)[i : i + 1600].tobytes() for i in range(0, 16000, 1600)]
    sink = _sink(DEVICE_RATE)
    piecewise = i16(b"".join(sink.convert(p, PACKET_RATE) for p in packets))
    continuous = i16(_sink(DEVICE_RATE).convert(whole, PACKET_RATE))
...
def test_reset_drops_the_filter_tail_without_touching_the_device() -> None:
    ...
    without_reset = i16(sink.convert(silence, PACKET_RATE))
    assert np.max(np.abs(without_reset)) > 1000  # the tail really does ring on
    sink.reset()
    after_reset = i16(sink.convert(silence, PACKET_RATE))
    assert not sink.stream.closed
    assert np.array_equal(after_reset, i16(_sink().convert(silence, PACKET_RATE)))
...
def test_a_new_model_rate_rebuilds_the_resampler() -> None:
    ...
    sink = _sink(DEVICE_RATE)
    sink.convert(sine(PACKET_RATE, 1600), PACKET_RATE)
    pcm = sine(24000, 2400)
    switched = sink.convert(pcm, 24000)
    assert i16(switched).size == 2400 * DEVICE_RATE // 24000  # 2x, no longer 3x
...
def test_the_conversion_saturates_instead_of_wrapping() -> None:
    ...
    out = i16(_sink().convert(pcm, PACKET_RATE))
...
def opened_streams(
    ...
    opened = record_opened_streams(playback_mod, "RawOutputStream", _FakeDevice)
    return opened
...
def _packets(
    ...
    per_packet = rate // 10
    whole = i16(sine(rate, per_packet * count))
    return [
        StreamPacket(
            session_id="ab" * 16,
...
async def test_playback_loop_opens_at_the_device_rate_and_converts_the_packets(
    ...
    reference = i16(_sink().convert(b"".join(p.pcm for p in packets), PACKET_RATE))
    played = i16(b"".join(written))
