...
async def test_a_new_model_rate_rebuilds_the_resampler_without_reopening(
    ...
    with pytest.raises(_EndOfTest):
        await network_playback_loop(StreamVcConfig(), _PacedTransport(first + second))
    assert len(sinks) == 1
    assert [i16(w).size for w in device.writes[:2]] == [4800, 4800]  # 1600 * 3
    assert [i16(w).size for w in device.writes[2:]] == [4800, 4800]  # 2400 * 2
...
async def test_prebuffer_blocks_are_silence_at_the_device_rate(
    ...
    assert all(sample == 0 for w in device.writes[:2] for sample in i16(w))
    assert any(sample != 0 for sample in i16(device.writes[2]))  # audio, not silence
