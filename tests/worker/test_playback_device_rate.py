...
from tests.pcm import i16
from tests.pcm import peak_frequency
from tests.pcm import sine
...
def stubbed_device_table(stub_device_table: Any) -> None:
    """The device table alone, without also patching sd.RawOutputStream.

    Kept separate from `opened_streams` so a test that patches the open itself can depend
    on the table without also installing an opener spy it never uses.
    """
...
def opened_streams(
    ...
    return record_opened_streams(playback_mod, "RawOutputStream", _FakeDevice)
...
def test_output_device_is_opened_at_the_resolved_native_rate(
    ...
    with caplog.at_level(logging.INFO):
        output = _opened()
        output.convert(sine(TTS_RATE, 240), TTS_RATE, SampleFormat.INT16, 1)
...
def test_a_source_rate_change_does_not_reopen_the_device(
    ...
    for rate in (TTS_RATE, VC_RATE, TTS_RATE, VC_RATE):
        output.update_stream_if_changed(format=SampleFormat.INT16, channels=1)
        output.convert(sine(rate, rate // 10), rate, SampleFormat.INT16, 1)
...
def test_a_reopen_drops_the_cached_resamplers(
    ...
    output = _opened()
    output.convert(sine(TTS_RATE, 2400), TTS_RATE, SampleFormat.INT16, 1)
...
def test_an_utterance_is_converted_to_the_device_rate(
    ...
    output = _opened()
    pcm = sine(TTS_RATE, 2400)
    out = output.convert(pcm, TTS_RATE, SampleFormat.INT16, 1)
    assert i16(out).size == 2400 * DEVICE_RATE // TTS_RATE
    assert abs(peak_frequency(out, DEVICE_RATE) - 440.0) < 3.0
    ...
    assert abs(peak_frequency(out, TTS_RATE) - 440.0) > 100.0
...
def test_the_converted_utterance_has_its_nominal_length(
    ...
    out = output.convert(sine(rate, samples), rate, SampleFormat.INT16, 1)
    assert i16(out).size == round(samples * DEVICE_RATE / rate)
...
def test_the_utterance_is_not_shifted_in_time_and_keeps_its_tail(
    ...
    step[1200:] = 20000
    out = i16(output.convert(step.tobytes(), TTS_RATE, SampleFormat.INT16, 1))
...
def test_the_same_utterance_twice_gives_the_same_bytes(
    ...
    after = i16(output.convert(silence, TTS_RATE, SampleFormat.INT16, 1))
...
def test_the_conversion_saturates_instead_of_wrapping(
    ...
    out = i16(output.convert(square.tobytes(), TTS_RATE, SampleFormat.INT16, 1))
...
def test_multichannel_utterances_keep_their_channel_layout(
    ...
    stereo[0::2] = 20000
    stereo[1::2] = -20000
    out = i16(output.convert(stereo.tobytes(), TTS_RATE, SampleFormat.INT16, 2))
...
def test_the_resampler_cache_is_bounded(opened_streams: list[_FakeDevice]) -> None:
    ...
    for rate in rates:
        output.convert(sine(rate, 800), rate, SampleFormat.INT16, 1)
...
def test_the_cache_keeps_the_rates_that_are_actually_in_use(
    ...
    with caplog.at_level(logging.INFO):
        for i in range(MAX_CACHED_RESAMPLERS * 2):
            output.convert(sine(TTS_RATE, 800), TTS_RATE, SampleFormat.INT16, 1)
            one_off = 9000 + i * 1000
            output.convert(sine(one_off, 800), one_off, SampleFormat.INT16, 1)
    builds = [
        r.getMessage()
...
def test_a_refused_rate_costs_a_full_cache_nothing(
    ...
    with pytest.raises(ValueError, match="病的"):
        output.convert(sine(44101, 800), 44101, SampleFormat.INT16, 1)
...
async def test_the_volume_is_applied_to_the_converted_audio_too(
    ...
    quiet = i16(opened_streams[0].writes[0])
    loud = i16(output.convert(pcm, TTS_RATE, SampleFormat.INT16, 1))
...
async def test_the_worker_plays_alternating_rates_through_one_open(
    ...
    tts = sine(TTS_RATE, 2400)
    vc = sine(VC_RATE, 4000)
    ...
    assert len(opened_streams) == 1
    assert opened_streams[0].kwargs["samplerate"] == DEVICE_RATE
    written = [i16(w).size for w in opened_streams[0].writes]
...
async def test_the_worker_still_records_e2e_telemetry(
    ...
    queue: Queue[WorkerInput] = Queue()
    queue.put_nowait(_speech(_sound(sine(TTS_RATE, 2400), TTS_RATE), origin_ts=1.0))
...
async def test_a_device_fault_during_the_write_is_warned_about_and_the_loop_goes_on(
    ...
    queue: Queue[WorkerInput] = Queue()
    for _ in range(2):
        queue.put_nowait(_speech(_sound(sine(TTS_RATE, 2400), TTS_RATE)))
...
async def test_a_pathological_utterance_rate_warns_and_the_next_utterance_plays(
    ...
    """
    queue: Queue[WorkerInput] = Queue()
    queue.put_nowait(_speech(_sound(sine(44101, 4410), 44101)))
    queue.put_nowait(_speech(_sound(sine(TTS_RATE, 2400), TTS_RATE)))
    ...
    assert len(opened_streams) == 1
    assert [i16(w).size for w in opened_streams[0].writes] == [4800]
    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert len(warnings) == 1
