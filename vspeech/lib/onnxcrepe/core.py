...
def predict(
    ...
    # Preprocess audio
    generator = preprocess(audio, sample_rate, precision, batch_size, pad)
    for frames in generator:
        ...
        result = postprocess(probabilities, fmin, fmax, decoder, return_periodicity)
        ...
        with open(os.path.join(output_directory, f"{title}.pitch.csv"), "w") as f:
            if save_periodicity:
                ...
                for i in range(prediction.shape[1]):
                    # time, f0
                    print("%f,%f" % (i * precision / 1000.0, prediction[0][i]), file=f)
    elif format == "npy":
        np.save(os.path.join(output_directory, f"{title}.f0.npy"), prediction[0])
        if save_periodicity:
            np.save(
                os.path.join(output_directory, f"{title}.periodicity.npy"),
                prediction[1],
            )
...
def preprocess(audio, sample_rate, precision=None, batch_size=None, pad=True):
    ...
    hop_length = (
        SAMPLE_RATE / 100 if precision is None else SAMPLE_RATE * precision / 1000
    )
    ...
    for i in range(0, total_frames, batch_size):
        ...
        end = min(audio.shape[0], int((i + batch_size - 1) * hop_length) + WINDOW_SIZE)
        ...
        frames = np.lib.stride_tricks.as_strided(
            ...
            strides=(int(hop_length) * n_bytes, n_bytes),
        )  # shape=(batch, 1024)
