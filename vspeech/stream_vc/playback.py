...
def open_stream_vc_output(config: StreamVcConfig) -> OutputSink:
    ...
    shape -- here including the requested latency (ADR-0071) -- and the pairing with the
    converter are decided here.
    ...
    def _open(rate: int) -> sd.RawOutputStream:
        stream = sd.RawOutputStream(
            samplerate=rate,
            channels=1,
            device=device.index,
            dtype="int16",
            # Handed to sounddevice untouched: a float is seconds, PortAudio's own unit
            # for suggestedLatency (ADR-0071).
            latency=config.output_latency,
        )
        # PortAudio does not have to honour the request, and "low" resolves to a
        # different number per host API, so the granted value is the only one worth
        # tuning against. Logged from inside the opener, not after open_device_stream
        # returns: that helper start()s the stream, and a failing start would otherwise
        # take the number with it. consumer.py reuses this function, so the consumer
        # machine gets the same line for its own speaker.
        logger.info(
            "stream_vc output stream latency: %.3fs granted (%s requested)",
            stream.latency,
    ...
    logger.info("stream_vc output stream latency: %s requested", config.output_latency)
