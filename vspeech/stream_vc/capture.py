...
def open_stream_vc_input_stream(config: StreamVcConfig, hop: int) -> InputTap:
    ...
    own shape -- here including the requested latency (ADR-0071) -- are decided here.
    ...
    def _open(rate: int) -> sd.RawInputStream:
        ...
        # take the number with it. The device, its rate and how that rate was decided
        # are already on the line open_device_stream logs before this runs.
        logger.info(
            "stream_vc input stream latency: %.3fs granted (%s requested)",
            stream.latency,
