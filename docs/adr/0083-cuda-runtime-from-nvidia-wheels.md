...
## 追記2（ADR-0081 の実装で判明した数値的帰結）

供給元を移すと、**同じ ONNX グラフの推論結果が bit 一致しなくなる。** cuBLAS / cuDNN のビルドが違えばカーネル選択が違い、fp16 の丸めが変わるためで、欠陥ではない。ただし ADR-0080 が定めた「変更前と bit 一致」の判定にはそのまま影響するので、実測値を残す。
