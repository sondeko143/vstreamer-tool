...
- `SetInputSettings` に渡す値の形式は OBS のバージョンに依存する実装詳細なので、`lib/obs_text_settings.py` に閉じ込める。**実装前に実機（OBS 32.1.2 / obs-websocket 5.7.3）で測定した結果、ここは実際に外れていた**: `color` は素の RGB ではなく **`0x00BBGGRR`（BGR）** で保存される。`0xFF8000` を書き込むと `rgb(0,128,255)` にレンダリングされ、`0x0080FF` で `rgb(255,128,0)` になる（`GetSourceScreenshot` のピクセルで測定。目視ではない）。obs-studio の `obs-text.cpp` は保存値を `rgb_to_bgr()` に通してから GDI+ に渡すが、その向きをソース読解から推定するのは誤りだった。同時に確認した: `inputKind` は `text_gdiplus_v3` で `SetInputSettings` は問題なく通る、`align` / `valign` は文字列を受ける、`font.flags` の bold ビットは `1`。
- **出荷デフォルトの `font_color = "#ffffff"` と `outline_color = "#000000"` は回文なので、バイト順を取り違えても症状が出ない。** 実測でも `#ffffff` は反転の有無にかかわらず正しくレンダリングされた。この並びを守れるのは非対称な色（`#ff8000` 等）のテストだけであり、そのテストが消えるとバグは既定値の下に隠れて再発する。
