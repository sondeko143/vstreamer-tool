...
- **開通確認を、実際の worker と全く同じ blocksize/channels/dtype/latency で行う** — `playback`(発話系)は実際の format/channels が config に無く実行時の発話ごとに決まるため、そもそも worker と同一の形状を preflight から再現できない。4境界とも channels=1・dtype=int16(recording のみ config の値)の最小限のプローブで統一し、検証するのは「このレートで開けるか」に絞る。`latency` もこの意図的な形状差の一つ: `stream_vc` の入出力は設定値 `stream_vc.input_latency` / `stream_vc.output_latency`(既定 `"low"`、[0071](0071-stream-vc-device-latency-config.md))で開く(`stream_vc/capture.py`, `stream_vc/playback.py`)が、preflight のプローブは既定 latency で開く。低レイテンシ要求(あるいは設定された明示秒数)だけを拒否するデバイスは preflight を通過し、worker の open で初めて落ちる — 実在確率は低いと見て、この差も他の形状差と同じ扱いのまま残す。
