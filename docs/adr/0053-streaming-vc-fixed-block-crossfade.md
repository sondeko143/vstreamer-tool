...
フェード則は **SOLA 導入後に再測定して sum-to-1 に切り替えた**。当初は SOLA 無しの無相関状態(lag 0 相関 -0.02)で測り「等電力=RMS 比 1.0007 / sum-to-1=0.873(-1.16dB ディップ)」だったので等電力を選んでいたが、**SOLA は意図的に信号を相関させる**(整列点相関 ρ=0.82)ので、この前提は void になった。相関した信号では等電力(²和=1)は seam を過剰加算する。そこで SOLA on で 3 則を同一 seed(=デコーダ出力・SOLA 整列がビット同一、重みだけ差)で再測定した:

| フェード則 | blend/rest RMS 比 | 6.25Hz 変調指数 |
|---|---|---|
| 等電力(旧) | +1.14 dB | 0.0671 |
| sum-to-1 (sin²/cos², 採用) | -0.76 dB | 0.0656 |
| linear | -0.90 dB | 0.0652 |

静的な blend/rest 比では sum-to-1 が僅かに優(等電力は予測どおり +1.14dB の seam bump = 過剰加算)。実際に聴こえる周期変調(6.25Hz)は 3 則でほぼ同一(0.067 vs 0.066、差 ~0.01dB)で、seam 連続性も 3 則とも mid-block より良好。3 則の WAV を実機で A/B したが**聴き分け不能**だった(2026-07-24)。→ **可聴差ゼロだが、より正しい sum-to-1(振幅保存、和=1)を採用**した: SOLA で相関させた seam には和=1 が正しく(²和=1 の等電力は相関信号を過剰加算する)、reference 実装の w-okada VCClient も SOLA に sum-to-1 を組む。可聴コスト無しで correctness と reference 整合の両方が得られるので切り替える。6.25Hz 変調が 3 則でほぼ同一だったのは、SOLA の整列が短窓で crossfade 帯全体では相関がやや薄れるため、+1.14dB の静的 bump が周期変調としては表面化しなかった、というのが解釈。
...
- **固定位置(lag 0)のままクロスフェードする** — 上の実測どおり lag 0 の相関は -0.02(ほぼ無相関〜逆相関)で、混ぜると comb になる。クロスフェード長を伸ばしても位相不整合は消えないので解決しない。
- **クロスフェード則を等電力(sin/cos、²和=1)のままにする** — SOLA 導入前の無相関前提(lag 0 相関 -0.02)では総電力一定の等電力が正しかったが、SOLA は整列点で信号を相関させる(ρ=0.82)ので前提が void。相関信号に等電力を使うと seam を過剰加算する(SOLA on 再測定で +1.14dB、sum-to-1 は -0.76dB でユニティ寄り)。可聴差は実機 A/B で無かった(上「実測」節)が、可聴コスト無しでより正しい振幅保存(sum-to-1、和=1)が得られ、reference の w-okada VCClient も SOLA に sum-to-1 を組むので、等電力は採らない。位相ずれの本質的な対策は SOLA でありフェード則ではないが、SOLA と組むフェード則としては sum-to-1 が正しい。
