...
20 秒で切れば最悪でも約 85 秒（google.auth 自身の ExponentialBackoff ぶんを含む）と、
retry を入れた後でも**修正前より短い**。

同期の refresh をイベントループから追い出すのも同じ理由で必須になる。
`sender.get_channel` の初回 refresh は async `_send` から呼ばれており、そのまま
だとループ全体が止まる（実測 1.5 秒、最悪では上の 85 秒）。retry を足して最悪時間を
伸ばす以上、ここは `to_thread` へ出さないと ADR-0004 の「宛先間ブロックが消えた」
性質も ADR-0035 の冷起動復帰も無意味になる。そのために `get_channel` は async にした。
...
- **1 回の更新が張る HTTP は 1 本とは限らない**。`use_ce_credentials` の
  ID トークンは `iam.Signer` がメタデータサーバへの GET と
  iamcredentials.googleapis.com への signBlob POST を、トークン POST より
  **先に** 行う。`get_id_token_credentials` の `Request` にも session を
  渡さないと、3 本のうち 1 本しか直らない。同じ理由で `build_auth_session` は
  `http://` にも同じ adapter を張る（メタデータサーバは平文 http）。
