...
**ここで ADR-0042 の設計リスクを 1 つ潰す（Task 7 のレビューが提起）。** obs-websocket が
「OBS プロセスは起きたがシーンコレクションはまだ読み込み中」の窓で接続を受け付けるなら、その
瞬間の `GetInputSettings` はソース不在＝**code 600** を返す。ADR-0042 はそれを「観測できた・
リトライしても直らない」と解釈して `WorkerStartupError` にするので、**まさに「vspeech を先に
起動して後から OBS を上げる」という ADR-0042 が守ろうとした運用で、音声パイプラインごと落ちる**。
実装は ADR に忠実なので、これはコードのバグではなく**決定のリスク**であり、実機でしか分からない。

確認手順: vspeech を `worker_type = "OBS"` で起動して待たせ、その状態で OBS を**コールドスタート**
する（シーンが多いプロファイルほど窓が広い）。ログを見る:
- `subtitle worker [obs] connected to ...` の後にそのまま字幕が出る → リスクは顕在化しない。
- `[subtitle] ... が存在しません` で exit する → **リスクは実在**。ADR-0042 を supersede し、
  ソース不在は「N 回連続で観測したら fail-loud」等へ緩める必要がある（時間で推測しない、という
  ADR-0042 自身の原則と衝突するので、決定の作り直しになる）。
