...
Expected: 既存の accepted 例外（audit の torch CVE 等）以外は緑。注: `scripts/tests/test_setup_models.py` は `testpaths=["tests"]` により既定スイープ外で `poe check` には含まれない。明示実行で確認する: `uv run --no-sync pytest scripts/tests/test_setup_models.py`。
