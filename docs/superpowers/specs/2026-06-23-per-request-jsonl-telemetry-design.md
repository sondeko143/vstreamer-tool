...
- **空＝無効（既定オフ）**。非空＝そのパスへ追記。`log_file` と同様に strftime 適用（例 `./telemetry_%%Y_%%m_%%d.jsonl`）。
- `Telemetry.configure(enabled, max_samples, jsonl_path="")` がパスを受け、非空かつ `enabled` 時に append モードでファイルを開いて保持。
- 書込: `json.dumps(record) + "\n"` を write→flush（発話レートで低頻度なので1行ごとflush＝クラッシュ耐性）。
- 失敗時（オープン/書込）は `logger.warning` で1回通知し、以後 JSONL を無効化（パイプラインは止めない）。
...
`main.py cmd` の `telemetry.configure(...)` に `jsonl_path=config.telemetry.jsonl_path` を追加。strftime 適用は **`configure` 内**で `datetime.now().strftime(jsonl_path)` を行ってからファイルを開く（呼び出し側は生パターンを渡すだけ）。`config.toml.example` に `jsonl_path` を記載。
...
6. 既存 `test_telemetry.py`（summary 等）が green 維持（trace_id 追加は後方互換）。
...
- 変更: [vspeech/lib/telemetry.py](../../../vspeech/lib/telemetry.py)（`record`/`record_e2e`/`timer` に trace_id、`_emit_jsonl`、`configure` に jsonl_path）、[vspeech/config.py](../../../vspeech/config.py)（`TelemetryConfig.jsonl_path`）、[vspeech/main.py](../../../vspeech/main.py)（configure 配線）、各ワーカー（transcription/translation/tts/vc/playback: trace_id を渡す）、`config.toml.example`（jsonl_path 記載）。
- 追加: `tests/test_telemetry_jsonl.py`。
